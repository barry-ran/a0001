...
<body>
<div class="container">
<script type="text/javascript">parent.document.title = \'零度分发平台 Board 管理中心 - 工具 - 清理缓存\';if(parent.$(\'admincpnav\')) parent.$(\'admincpnav\').innerHTML=\'工具&nbsp;&raquo;&nbsp;清理缓存\';</script>
...
function main()
{
	...
	$webview = $db->num_rows($db->query("select count(*) from " . tname('webview') . " where in_status=1"));
	$webviewlog = $db->num_rows($db->query("select count(*) from " . tname('webviewlog') . " where in_status=1"));
	if (!is_dir('data/tmp/')) {
		$tmp = " disabled";
	}
	if (!$salt && !$mail && !$sign && !$signlog && !$webview && !$webviewlog) {
		$sql = " disabled";
	}
...
function save()
{
	...
	if ($sql == 1) {
		...
		$db->query("delete from " . tname('webview') . " where in_status=1");
		$db->query("delete from " . tname('webviewlog') . " where in_status=1");
	}
