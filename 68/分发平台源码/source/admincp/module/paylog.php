...
<script type="text/javascript">parent.document.title = \'零度分发平台 Board 管理中心 - 系统 - 充点记录\';if(parent.$(\'admincpnav\')) parent.$(\'admincpnav\').innerHTML=\'系统&nbsp;&raquo;&nbsp;充点记录\';</script>
...
<option value="?iframe=paylog&lock=0"';
	if (isset($_GET['lock']) && $_GET['lock'] == 0) {
		echo " selected";
	}
	echo '>支付成功</option>
<option value="?iframe=paylog&lock=1"';
...
<td>';
			...
			echo '</td>
<td><a href="?iframe=paylog&action=keyword&key=';
			echo $row['in_uname'];
			echo '" class="act">';
