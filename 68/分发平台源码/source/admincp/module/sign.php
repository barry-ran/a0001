...
<script type="text/javascript">parent.document.title = \'零度分发平台 Board 管理中心 - 应用 - 签名管理\';if(parent.$(\'admincpnav\')) parent.$(\'admincpnav\').innerHTML=\'应用&nbsp;&raquo;&nbsp;签名管理\';</script>
...
<option value="?iframe=sign&status=1"';
	if (isset($_GET['status']) && $_GET['status'] == 1) {
		echo " selected";
	}
	echo '>正在签名</option>
<option value="?iframe=sign&status=2"';
...
';
	if ($count == 0) {
		echo '<tr><td colspan="2" class="td27">没有签名</td></tr>
';
	} else {
...
<td>';
			...
			echo '</td>
<td><a href="';
			echo $row['in_ssl'] . $row['in_site'] . $row['in_path'] . 'app.php/' . auth_codes($row['in_aid']);
			...
			echo ']</a></td>
<td>';
			echo str_replace(SafeRequest("key", "get"), '<em class="lightnum">' . SafeRequest("key", "get") . '</em>', $row['in_site']);
			echo '</td>
<td>';
