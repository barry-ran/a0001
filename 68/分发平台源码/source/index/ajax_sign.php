...
if ($ac == 'purchase') {
	...
	$GLOBALS['db']->query("update " . tname('app') . " set in_sign={$time},in_resign=in_resign+" . IN_RESIGN . " where in_id=" . $aid);
	echo '1';
} elseif ($ac == 'download') {
	$aid = intval(SafeRequest("aid", "get"));
	$row = $GLOBALS['db']->getrow("select * from " . tname('app') . " where in_id=" . $aid);
	...
	inserttable('salt', array('in_aid' => $aid, 'in_salt' => $salt, 'in_time' => time()));
	...
	$aid = intval(SafeRequest("aid", "get"));
	$cert = rawurldecode(SafeRequest("cert", "get"));
	$row = $GLOBALS['db']->getrow("select * from " . tname('app') . " where in_id=" . $aid);
	$row or exit('-2');
	$row['in_uid'] == $GLOBALS['erduo_in_userid'] or exit('-3');
	preg_match('/(\\.ipa)$/', $row['in_app']) or exit('-4');
	...
	$setarr = array('in_aid' => $row['in_id'], 'in_aname' => $row['in_name'], 'in_uid' => $GLOBALS['erduo_in_userid'], 'in_uname' => $GLOBALS['erduo_in_username'], 'in_status' => 1, 'in_step' => 'unknown', 'in_percent' => 0, 'in_cert' => $cert, 'in_addtime' => date('Y-m-d H:i:s'));
	if ($status) {
