...
abstract class CFBinaryPropertyList 
{
	...
	protected function readBinaryInt($length) 
	{
		...
		switch($length) 
		{
			case 0: $val = unpack("C",$buff);
			$val = $val[1];
			break;
			case 1: $val = unpack("n",$buff);
			$val = $val[1];
			break;
			case 2: $val = unpack("N",$buff);
			$val = $val[1];
			break;
			case 3: $words = unpack("Nhighword/Nlowword",$buff);
	...
	protected function readBinaryReal($length) 
	{
		...
		switch($length) 
		{
			case 0: case 1: $x = $length +1;
			throw new PListException("got {$x}
		byte float, must be an error!");
		case 2: $val = unpack("f",strrev($buff));
		$val = $val[1];
		break;
		case 3: $val = unpack("d",strrev($buff));
		$val = $val[1];
		break;
	}
	return new CFNumber($val);
...
switch($object_type) 
{
	case '0': $retval = $this->readBinaryNullType($object_length);
	break;
	case '1': $retval = $this->readBinaryInt($object_length);
	break;
	case '2': $retval = $this->readBinaryReal($object_length);
	break;
	case '3': $retval = $this->readBinaryDate($object_length);
	break;
	case '4': $retval = $this->readBinaryData($object_length);
	break;
	case '5': $retval = $this->readBinaryString($object_length);
	break;
	case '6': $retval = $this->readBinaryUnicodeString($object_length);
	break;
	case '8': $num = $this->readBinaryInt($object_length);
	$retval = new CFUid($num->getValue());
	break;
	case 'a': $retval = $this->readBinaryArray($object_length);
	break;
	case 'd': $retval = $this->readBinaryDict($object_length);
	break;
}
return $retval;
}
...
{
$intbytes = "";
if($int >0xFFFF) $intbytes = "".pack("N",$int);
elseif($int >0xFF) $intbytes = "".pack("n",$int);
else $intbytes = "".pack("C",$int);
return $intbytes;
}
...
$object_offset = 8;
$offsets = Array();
for($i=0;$i<count($this->objectTable);
++$i) 
{
