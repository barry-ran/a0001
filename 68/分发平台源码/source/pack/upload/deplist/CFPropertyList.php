...
class CFPropertyList extends CFBinaryPropertyList implements Iterator 
{
	...
	public function load($file=null,$format=null) 
	{
		...
		switch($format) 
		{
			case CFPropertyList::FORMAT_BINARY: $this->readBinary($file);
			break;
			case CFPropertyList::FORMAT_AUTO: $fd = fopen($file,"rb");
			...
			case CFPropertyList::FORMAT_XML: $doc = new DOMDocument();
	...
	public function parse($str=NULL,$format=NULL) 
	{
		...
		switch($format) 
		{
			case CFPropertyList::FORMAT_BINARY: $this->parseBinary($str);
			break;
			case CFPropertyList::FORMAT_AUTO: if(($magic_number = substr($str,0,8)) === false) throw IOException::notReadable("<string>");
			$filetype = substr($magic_number,0,6);
			$version = substr($magic_number,-2);
			if($filetype == "bplist") 
			{
	...
	protected function import(DOMNode $node,$parent) 
	{
		...
		foreach($node->childNodes as $n) 
		{
			...
			switch($n->nodeName) 
			{
				case 'date': $value = new $class(CFDate::dateValue($n->nodeValue));
				break;
				case 'data': $value = new $class($n->nodeValue,true);
				break;
				case 'string': $value = new $class($n->nodeValue);
				break;
				case 'real': case 'integer': $value = new $class($n->nodeName == 'real'?floatval($n->nodeValue) : intval($n->nodeValue));
				break;
				case 'true': case 'false': $value = new $class($n->nodeName == 'true');
				break;
				case 'array': case 'dict': $value = new $class();
				$this->import($n,$value);
	...
	public function save($file=null,$format=null) 
	{
		...
		if( !in_array( $format,array( self::FORMAT_BINARY,self::FORMAT_XML ) ) ) throw new PListException( "format {$format}
	is not supported, use CFPropertyList::FORMAT_BINARY or CFPropertyList::FORMAT_XML");
	if(!file_exists($file)) 
	{
		if(!is_writable(dirname(__FILE__))) throw IOException::notWritable($file);
	}
...
{
	static $t = null;
	if( $t === null ) $t = new CFTypeDetector( $options );
