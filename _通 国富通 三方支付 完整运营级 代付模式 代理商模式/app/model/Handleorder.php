...
class Handleorder extends Model
{
    ...
    public function updateUncard()
    {
		...
		if($this->orderid){
			$orderid=explode("_",$this->orderid);
			if($orderid[0]=='fastbank'){
				$orderid=$orderid[1];
				$orderid=$this->model()->select()->from('orders')->where(array('fields' => 'sdorderno=?', 'values' => array($orderid)))->fetchRow();
			
				$this->orderid=$orderid['orderid'];
			}
		}
		if(!$this->money){
			$o=$this->model()->select()->from('orders')->where(array('fields' => 'orderid=?', 'values' => array($this->orderid)))->fetchRow();
			
			$this->money=$o['total_fee'];
