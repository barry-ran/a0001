...
class ucfastbanksubmit extends apifast
{
    ...
    protected function submit()
    {
        ...
        if($_POST['command']==2){
			$url = 'http://' . $this->req->server('HTTP_HOST') . '/pay/' . $acpcode . '_' . $gateway . '/send.php';
			$url .= '?command=GWQP002&orderNum='.$_POST['orderNum'].'&contractId='.$_POST['contractId'].'&payMoney='.$_POST['payMoney'].'&notifyUrl='.$_POST['notifyUrl'].'&memberId='.$_POST['memberId'];
