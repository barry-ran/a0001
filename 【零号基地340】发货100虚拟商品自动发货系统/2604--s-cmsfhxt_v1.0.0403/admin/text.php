...
if($T_id!=""){
	$aa="edit&T_id=".$T_id;
	$sql="select * from sl_text where T_id=".$T_id;
...
if($T_title!=""){
	if(getrs("select * from sl_text where T_title='$T_title' and T_del=0","T_id")==""){
		mysqli_query($conn,"insert into sl_text(T_pic,T_title,T_order,T_content,T_type,T_zb,T_address) values('$T_pic','$T_title',$T_order,'$T_content',$T_type,'$T_zb','$T_address')");
...
if($action=="delall"){
	...
	if(count($id)>0) {
		...
		for ($i=0 ;$i<count($id);$i++ ) {
			mysqli_query($conn,"update sl_text set T_del=1 where T_id=".intval($id[$i]));
			$shu=$shu+1 ;
			$ids=$ids.$id[$i].",";
		}
...
$sql="select count(T_id) as T_count from sl_text where T_del=0";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$T_counts=$row["T_count"];
?>
<!DOCTYPE html>
<html lang="en">
	<head>
						...
						<div class="section-body ">
							...
							<div class="row">
								...
								<div class="col-lg-4">
										...
										<div class="card-header">
											<h4>单页列表</h4>
												...
												<ul class="list-group">
													...
													<?php 
														$sql="select * from sl_text where T_del=0 order by T_order,T_id desc limit ".(($page-1)*20).",20";
																$result = mysqli_query($conn, $sql);
																if (mysqli_num_rows($result) > 0) {
																while($row = mysqli_fetch_assoc($result)) {
																	...
																	echo "<a id=\"".$row["T_id"]."\" href=\"?T_id=".$row["T_id"]."\" class=\"list-group-item ".$active."\">
																	<div class=\"part\"><input type=\"checkbox\" name=\"id[]\" value=\"".$row["T_id"]."\"> ".$row["T_order"].".".$row["T_title"]."</div> 
																	<div class=\"part\">".$type."</div>
																	
																	<img src=\"../media/".$row["T_pic"]."\" alt=\"<img src='../media/".$row["T_pic"]."' width='300'>\" style=\"height:25px;border-radius:10px;\" class=\"pull-right\"></a>";
																}
															}
													?>
									...
									<a href="text.php" class="btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> 新增</a>
									...
									<div class="card card-primary">
										...
										<div class="card-body">
												...
												<div class="form-group row">
													...
													<div class="col-md-9">
														<p><img src="../media/<?php echo $T_pic?>" id="T_picx" class="showpic" onClick="showUpload('T_pic','T_pic','../media',1,null,'','');" alt="<img src='../media/<?php echo $T_pic?>' class='showpicx'>"></p>
