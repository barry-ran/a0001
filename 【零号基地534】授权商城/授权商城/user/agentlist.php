...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_user where  user_belong = '".$userrow['username']."' and ( username  like '%$pz%' or user_qq like '%$pz%' or user_name like '%$pz%' )");
}else{
 
    $numrows=$DB->count("SELECT count(*) from ku_user where  user_belong = '".$userrow['username']."'");
}
$pagesize=20;
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_user WHERE  user_belong = '".$userrow['username']."' and ( username  like '%$pz%'  or user_qq like '%$pz%' or user_name like '%$pz%' ) order by user_addtime desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_user where  user_belong = '".$userrow['username']."' order by user_addtime desc limit $offset,$pagesize";
}
?>
 <div class="main">
                    ...
                    while($res = $DB->fetch($rs))
                   {
                    ?>
                      <tr> 
                        ...
                        <td>
                       <a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$res['user_qq']?>&Site=酷享授权&Menu=yes"><?=$res['user_qq']?></a>
                        </td>
                        <td><?=daili_lv($res['user_limit'])?></td>
                        <td><?=$res['squan_limit']?></td>
                        <td><?=daili_sta($res['user_status'])?></td>
                        <td><span style="color:#436EEE"><?=$res['user_belong']?></span></td>
                        <td>
                        <a href="agentedit.html?my=edit&id=<?=$res['id']?>" title="修改">
