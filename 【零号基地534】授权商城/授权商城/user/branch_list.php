...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_cons where  user_belong = '{$userrow['username']}' and data= 1 and ( out_trade_no  like '%$pz%' or username like '%$pz%')");
}else{
 
    $numrows=$DB->count("SELECT count(*) from ku_cons where  user_belong = '{$userrow['username']}' and data= 1");
}
$pagesize=20;
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_cons WHERE  user_belong = '{$userrow['username']}' and data= 1 and ( out_trade_no  like '%$pz%'  or username like '%$pz%') order by addtime desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_cons where  user_belong = '{$userrow['username']}' and data= 1 order by addtime desc limit $offset,$pagesize";
}
?>
 <div class="main">
			<div class="main-content">
				<div class="container-fluid">
				  <h3 class="page-title">佣金明细</h3>
 	                    <div class="row">
