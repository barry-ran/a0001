...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_paycarry where  username = '{$userrow['username']}' and ( out_trade_no  like '%$pz%')");
}else{
 
    $numrows=$DB->count("SELECT count(*) from ku_paycarry where  username = '{$userrow['username']}'");
}
$pagesize=20;
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_paycarry WHERE  username = '{$userrow['username']}' and ( out_trade_no  like '%$pz%') order by addtime desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_paycarry where  username = '{$userrow['username']}' order by addtime desc limit $offset,$pagesize";
}
?>
 <div class="main">
				  ...
				  <h3 class="page-title">提现列表</h3>
 	                    ...
 	                    <div class="panel-body">
									 <div class="input-group col-md-4">
										<input class="form-control" type="text" name="pz" placeholder="请输入订单号查询" value="<?=$_POST['pz']?>" required="">
										...
										<tbody>
                                           <?php
											...
											<tr> 
												...
												<td><?=$res['addtime']?></td>
												<td><?=$res['money']?>/元</td>
												<td><?=$res['fee_money']?>/元</td>
												<td><?=$res['operation']?></td>
												<td><?=$res['eidetime']?></td>
												<td><?=$res['username']?></td>
												<td><?=pay_lv($res['state'])?></td>       
											</tr>
										<?php }?>
										</tbody>
									</table>
