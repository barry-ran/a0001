...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_carmlog where  username = '".$userrow['username']."' and ( carmine  like '%$pz%')");
}else{
 
    $numrows=$DB->count("SELECT count(*) from ku_carmlog where  username = '".$userrow['username']."'");
}
$pagesize=20;
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_carmlog WHERE  username = '".$userrow['username']."' and ( carmine  like '%$pz%') order by addtime desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_carmlog where  username = '".$userrow['username']."' order by addtime desc limit $offset,$pagesize";
}
?>
 <div class="main">
                    ...
                    while($res = $DB->fetch($rs))
                   {
                    ?>
                      <tr> 
                        <td><?=$res['kuhost']?></td>
                        <td><?=$res['carmine']?></td>
                        <td><a href="http://wpa.qq.com/msgrd?v=3&uin=<?=$res['ku_qq']?>&Site=余生网络信息&Menu=yes" title="点击直接QQ会话" style="color:#FF00FF"><?=$res['ku_qq']?> </a>
                        </td>
                        <td><?=$res['addtime']?></td>
                        <td><?=kami_log($res['status'])?></td>
                        <td><?=$res['username']?></td>
                      </tr>
                    <?php }?>
                    </tbody>
