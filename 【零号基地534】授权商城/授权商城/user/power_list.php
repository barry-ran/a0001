...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_power where  username = '".$userrow['username']."' and ( out_trade_no  like '%$pz%' or url like '%$pz%' or money like '%$pz%' )");
}else{
 
    $numrows=$DB->count("SELECT count(*) from ku_power where  username = '".$userrow['username']."'");
}
$pagesize=20;
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_power WHERE  username = '".$userrow['username']."' and ( out_trade_no  like '%$pz%'  or url like '%$pz%' or money like '%$pz%' ) order by addtime desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_power where  username = '".$userrow['username']."' order by addtime desc limit $offset,$pagesize";
}
?>
 <div class="main">
			<div class="main-content">
				<div class="container-fluid">
				  <h3 class="page-title">购买授权列表</h3>
 	                    <div class="row">
