...
use Yii;
class FaqController extends BController
{
    ...
    public function actionAjaxlist(){
        ...
        $res = MY_Faq::getList($pid, $typeid , $title);
...
//                    "hits" => $item["hits"],
    ...
    public function actionDelete(){
        ...
        if (empty($id)) {
            Yii::$app->getSession()->setFlash('success', "请选择要删除的常见问题！");
        } else {
            $faq = MY_Faq::find()->where("id = :id",[":id"=>$id])->one();
            $res = Yii::$app->db->createCommand() ->delete('me_faq',"id = $id") ->execute();
