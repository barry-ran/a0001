...
class MY_Article extends Article {
    /*
     * 设置表操作行为动作
     * return array
     */
    ...
    public static function getList($pid , $typeid , $title) {
        $query = MY_Article::find()->with(["typeID"])->orderBy("created_at desc");
    ...
    public static function createData($id = null) {
        $model = $id ? MY_Article::findOne($id) : new MY_Article();
        ...
        try {
            ...
            if($id){
                \common\models\Actionlog::setLog('编辑新闻：'.$model->title.'的信息');
            }else{
                \common\models\Actionlog::setLog('新增新闻：'.$model->title);
            }
