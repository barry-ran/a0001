...
class MY_Guide extends Guide {
    ...
    public static function getList($pid , $typeid , $title) {
        ...
        $query = MY_Guide::find()->orderBy("created_at desc");
    ...
    public static function createData($id = null) {
        ...
        $model = $id ? MY_Guide::findOne($id) : new MY_Guide();
        ...
        try {
            ...
            if($id){
                \common\models\Actionlog::setLog('编辑新手指南：'.$model->title.'的信息');
            }else{
                \common\models\Actionlog::setLog('新增新手指南：'.$model->title);
            }
