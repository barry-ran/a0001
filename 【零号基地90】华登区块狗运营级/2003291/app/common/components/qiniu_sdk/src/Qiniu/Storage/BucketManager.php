<?php
namespace common\components\qiniu_sdk\src\Qiniu\Storage;
use common\components\qiniu_sdk\src\Qiniu\functions;
use common\components\qiniu_sdk\src\Qiniu\Auth;
use common\components\qiniu_sdk\src\Qiniu\Config;
use common\components\qiniu_sdk\src\Qiniu\Zone;
use common\components\qiniu_sdk\src\Qiniu\Http\Client;
use common\components\qiniu_sdk\src\Qiniu\Http\Error;
...
final class BucketManager
{
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        functions::setWithoutEmpty($query, 'prefix', $prefix);
        functions::setWithoutEmpty($query, 'marker', $marker);
        functions::setWithoutEmpty($query, 'limit', $limit);
        functions::setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    {
        $path = '/delete/' . functions::entry($bucket, $key);
    ...
    {
        $from = functions::entry($from_bucket, $from_key);
        $to = functions::entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        if ($force === true) {
            $path .= '/force/true';
        }
    ...
    {
        $resource = functions::entry($bucket, $key);
        $encode_mime = functions::base64_urlSafeEncode($mime);
    ...
    {
        $entry = functions::entry($bucket, $key);
    ...
    public static function buildBatchDeleteAfterDays($bucket, $key_day_pairs)
    {
        ...
        foreach ($key_day_pairs as $key => $day) {
            array_push($data, '/deleteAfterDays/' . functions::entry($bucket, $key) . '/' . $day);
        }
        return $data;
    }
    ...
    public static function buildBatchChangeMime($bucket, $key_mime_pairs)
    {
        ...
        foreach ($key_mime_pairs as $key => $mime) {
            array_push($data, '/chgm/' . functions::entry($bucket, $key) . '/mime/' . base64_encode($mime));
        }
        return $data;
    }
    ...
    public static function buildBatchChangeType($bucket, $key_type_pairs)
    {
        ...
        foreach ($key_type_pairs as $key => $type) {
            array_push($data, '/chtype/' . functions::entry($bucket, $key) . '/type/' . $type);
        }
        return $data;
    }
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . functions::entry($bucket, $key));
        }
        return $data;
    }
