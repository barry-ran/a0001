<?php
class basklistAction extends BackendAction{
    ...
    protected function _search(){
        ...
        $map = array();
        ($time_start = $this -> _request('time_start', 'trim')) && $map['create_time'][] = array('egt', strtotime($time_start));
        ($time_end = $this -> _request('time_end', 'trim')) && $map['create_time'][] = array('elt', strtotime($time_end) + (24 * 60 * 60-1));
        ($status = $this -> _request('status', 'trim')) && $map['status'] = $status;
        if($status==2){
            $map['status']=0;
        }
        ($keyword = $this -> _request('keyword', 'trim')) && $map['content'] = array('like', '%' . $keyword . '%');
...
$mod=M('basklist_logo');
 ...
 if($_GET['keyword']){
     $where['order_sn'] = $this->_get('keyword', 'trim');
 }
 
$prefix = C(DB_PREFIX);
$field = '*,
   (select nickname from '.$prefix.'user where '.$prefix.'user.id = '.$prefix.'basklist_logo.uid) as nickname';
