<?php
class brand_cateAction extends BackendAction {
    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('brand_cate');
    }
    ...
    public function index() {
        ...
        foreach($result as $r) {
         if($r['id']!=2){
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('brand_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_article_cate').'" data-id="add" data-width="500" data-height="360">'.L('add_article_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('brand_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="290">'.L('edit').'</a> |
                                <a href="javascript:;" data-acttype="ajax" class="J_confirmurl" data-uri="'.U('brand_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
    ...
    public function ajax_upload_img() {
        ...
        if (!empty($_FILES['img']['name'])) {
            $result = $this->_upload($_FILES['img'], 'brand_cate', array('width'=>'80', 'height'=>'80'));
            if ($result['error']) {
