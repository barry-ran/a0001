<?php
class goodsjifenAction extends BackendAction {
    public function _initialize() {

        parent::_initialize();
        $this->_mod = D('goodsjifen');
    ...
    if (isset($_REQUEST['dosubmit'])) {
        if ($_REQUEST['isok'] == "1") {
            ...
            if ($time_start != '') {
                ...
                $where .= " AND end_time>='" . $time_start_int . "'";
            }
            if ($time_end != '') {
                $time_end_int = strtotime($time_end);
                $where .= " AND end_time<='" . $time_end_int . "'";
            }
            if ($status != '') {
            ...
            $this->success('删除成功', U('goodsjifen/clear'));
        } else {
            $this->success('确认是否要删除？', U('goodsjifen/clear'));
        }
    } else {
        $this->display();
    }
...
public function key_addtime(){
    if(IS_POST){
        ...
        $where = '1=1';
        $where.=" AND pass=1  AND end_time <".time();
        if('aday' == $action){
            $datas['end_time']=time()+86400;
        }elseif('twday' == $action){
            $datas['end_time']=time()+2*86400;
        }elseif('trday' == $action){
            $datas['end_time']=time()+3*86400;
        }else{
            $times	= $this->_post('times', 'intval');
            !$times && $this->error('请输入延时时间',U('goodsjifen/key_addtime'));
            $datas['end_time']=time()+$times*3600;
        }
        ...
        if ($datas) {
            ...
            $this->error('错误',U('goodsjifen/key_addtime'));
        }
    }else{
        $this->display();
    }
...
public function outtime(){
    if(IS_POST){
        $action	= $this->_post('action', 'trim');
        $where = '  pass=1  AND end_time <'.time();
...
public function add_time() {
    ...
    $datas['end_time']=time()+C('yh_item_add_time')*3600;
    if ($datas) {
...
public function getcheck(){
    if(IS_POST){
        ...
        if($pass == 0){
            ...
            $check_date['start_time'] = $start_time;
            $check_date['end_time'] = $end_time;
...
public function ajaxgetid() {
    ...
    $items = M('goodsjifen')->where(array('num_iid' => $iid))->find();
...
public function ajax_gettags() {
    ...
    $tag_list = D('goodsjifen')->get_tags_by_title($title);
