...
class indexAction extends BackendAction {
    ...
    public function panel() {
        ...
        $system_info = array(
            'yhxia_version' => FTX_VERSION . ' RELEASE '. FTX_RELEASE .' ',
 ...
 //不要删除域名否则无法获得系统更新提醒       
     $version = json_decode(file_get_contents('http://api.tuiquanke.com/version'), true);
        if($version['result']['version'] == FTX_VERSION){
            $system_info['yhxia_version'] = $version['result']['name'];
        }
        else{
            $system_info['yhxia_version'] = '<a href="'.$version['result']['url'].'" target="_blank">有新版本可以更新</a>';
        }
