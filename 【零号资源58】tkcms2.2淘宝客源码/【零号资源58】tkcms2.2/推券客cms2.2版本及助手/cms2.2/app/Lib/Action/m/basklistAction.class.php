<?php
class basklistAction extends FirstendAction
{
...
public function uploadPics(){
    if(IS_POST){
        if($_FILES['file']){
            $file = $this->_upload($_FILES['file'],'site/');
            $data = "/" . C("yh_attach_path") . 'site/' . $file['info'][0]['savename'];
            if($file){
                $this->ajaxReturn(array('error'=>0,'url'=>$data));
            }     else {
...
public function mylist(){
   ...
   $prefix = C('DB_PREFIX');
    ...
    $total_score = $this->mo->field('sum(integray) as integrays')->where($where)->select();
