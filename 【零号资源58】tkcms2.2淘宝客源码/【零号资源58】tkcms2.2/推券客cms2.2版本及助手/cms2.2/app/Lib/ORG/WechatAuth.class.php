...
class WechatAuth {
    ...
    public function getJsticket(){
    	...
    	return $this->api('ticket/getticket','','GET',$param);
    }
    ...
    /**
     * 获取模板ID
     * @param  $temp_id_short 模板库中模板的编号，有“TM**”和“OPENTMTM**”等形式
     * @return  <返回值描述>
     * @author 高友龙  <2015-4-21>
     */
    public function getTempID($temp_id_short)
    {
    	$data = array(
    			'template_id_short'=>$temp_id_short
    	);
     ...
     * @param  $keyword		模板变量关键词
     * @param  $url			详情地址
     * @param  $topcolor	top颜色
     * @return  <返回值描述>
     * @author 高友龙  <2015-4-21>
     */
    public function sendTempMsg($touser,$template_id,$keyword,$url,$topcolor='#FF0000')
    {
    	$key = array();
    ...
    public function getSession($openid)
    {
    	...
    	$apiUrl = 'https://api.weixin.qq.com/customservice/kfsession/getsession';
    	$data = $this->http($apiUrl, $params,'','GET');
    	return json_decode($data, true);
    }
     ...
     * @author 高友龙  <2015-4-23>
     */
    public function closeSession($openid,$kf_account,$text)
    {
    	$params = array('access_token' => $this->accessToken);
    	$data = array(
    			'openid'=>$openid,
    	...
    	$apiUrl = 'https://api.weixin.qq.com/customservice/kfsession/close';
    	$data = $this->http($apiUrl,$params,$data,'POST');
    	return json_decode($data, true);
    }
     ...
     * @author 高友龙  <2015-4-23>
     */
    public function createSession($openid,$kf_account)
    {
    	$params = array('access_token' => $this->accessToken);
    	$data = array(
    			'openid'=>$openid,
    	...
    	$apiUrl = 'https://api.weixin.qq.com/customservice/kfsession/create';
    	$data = $this->http($apiUrl,$params,$data,'POST');
    	return json_decode($data, true);
    }
    ...
    {
    	$params = array('access_token' => $this->accessToken);
    	$apiUrl = 'https://api.weixin.qq.com/customservice/kfsession/getwaitcase';
    	$data = $this->http($apiUrl,$params,'','GET');
    	return json_decode($data, true);
    }
