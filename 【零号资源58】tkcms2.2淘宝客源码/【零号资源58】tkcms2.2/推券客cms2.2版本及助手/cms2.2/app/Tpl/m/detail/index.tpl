	...
	<div class="detail">
		<div class="pic">
			...
			<a class="open_fullimg_btn" href="javascript:void(0)" id="example1">
				<div style="background-color:red;opacity:0.8;border-radius:50%;text-align: center;">分享</div>
			</a>
		...
		<div class="tit">
			<p class="ellipsis-2">
				<if condition="$item['shop_type'] eq 'B'">
					<img src="__STATIC__/tqkwap/images/tmall.png" width="20" height="auto">
				</if>
				<if condition="$item['shop_type'] eq 'C'">
					<img src="__STATIC__/tqkwap/images/taobao.png" width="20" height="auto">
				</if>	
		...
		<div class="price flexbox">
			<div class="txt">
				...
				<p><if condition="$item['shop_type'] eq 'B'">天猫</if><if condition="$item['shop_type'] eq 'C'">淘宝</if>在售:￥{$item.price}</p>
			</div>
			...
			<div class="detail canvas hidden">
				...
				<div class="price flexbox">
					<div class="tit">
						...
						<p style="font-size: 1.7rem"><if condition="$item['shop_type'] eq 'B'">天猫</if><if condition="$item['shop_type'] eq 'C'">淘宝</if>在售:￥{$item.price}<br><em>领券立省{$item.quan}元</em></p>
					</div>
					<div class="tag" style="background-size:130px 135px;background-image:url(__STATIC__/tqkwap/images/erweima.png);background-repeat:no-repeat;height: 136px;width: 133px;background-position:50% 50%; ">
						...
						{:urlencode($mdomain.'/index.php?g=m&m=detail&id='.$item['id'].'&trackid='.I('trackid'))}" width="110px" height="110px" style="margin-top:3px ">
					</div>
				</div>
			</div>
		</div>
		<div class="report am-cf">
			<ul>
				...
				<li>
					<a href="/index.php?g=m&m=baoming&a=jubao&num_iid={$item.num_iid}"><i class="iconfont icon-jubao"></i>举报此商品</a>
				</li>
			</ul>
		</div>
	</div>
			...
			<volist name="orlike" id="val">
				<li>
					<a rel="nofollow" href="{:U('detail/index',array('id'=>$val['id']))}{$trackurl}">
						<img src="{$val.pic_url}_300x300.jpg" width="100%"/>
...
<footer class="buy-nav flexbox">
	...
	<if condition="$act eq 'yes'">
		<a  href="/index.php?g=m&m=out&quanurl={:base64_encode($item['quanurl'])}"  class="mui-col-xs-4 browser" >领券下单</a>	
		<elseif condition="$item['que'] eq 1 || $item['tk'] eq 0"/>
		<a href="/index.php?g=m&m=out&id={$item.id}" class="mui-col-xs-4 browser" > 领券下单</a>
		<else/>
		<a id="coupon" href="javascript:;"  class="mui-col-xs-4 browser" >领券下单</a>
	</if>
	<a href="javascript:;" <if condition="$item['que'] eq 1 || $item['tk'] eq 0"> class="taobao" id="kouling"
		<else/> class="taobao" id="couponkouling"</if> data-am-modal="{target: '#amoybuy'}">用淘口令领券</a>
	</footer>
...
<yh:load type="js" href="__STATIC__/tqkwap/js/jquery.min.js,__STATIC__/tqkwap/js/amazeui.min.js,__STATIC__/tqkwap/js/clipboard.min.js,__STATIC__/tqkwap/js/loimsg.js" />
				...
				success: function(json){
					...
					html2canvas($(".hidden"), {  
						...
						onrendered: function(canvas) {
							...
							'</button>'+
							'<div class="am-text-xs am-margin-xs am-sm-only-text-left iptNum" onfocus="iptNum(this, true);">{$item.title}（包邮）<br>【原价】￥{$item.price}元 <br>【券后】￥{$item.coupon_price}元<br>复制这条信息，打开「手机淘宝」即可领劵下单！{$item.quankouling}'+
							'</div>'+
                           // '<input id="keleTkl" style="height:1px;" value="{$item.title}（包邮）【原价】￥{$item.price}元 【券后】￥{$item.coupon_price}元 复制这条信息，打开「手机淘宝」即可领劵下单！{$item.quankouling}"/>'+
							'</div>'+
